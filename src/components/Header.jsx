...
function Header() {
  ...
  const changeTheme = () => {
    ...
    if(theme) {//if the theme is dark we make the background dark, and the texts color white
      root.style.backgroundColor = "black";
      root.style.color = "#fff";
    } else { //if it is the opposite we make the background white, and the texts color black
  ...
  return (
    ...
    <div className='flex-row'>{/**the div keeps logo and brand */}
        <img src="/src/images/logo.png" className='logo' alt="#" />
        <p className='brand-text' >MALLWORLD C.</p>
    </div>
 
    <div className='flex-row'>{/**it keeps input and icons */}
      <input className='search-input' type="text" placeholder='search something' />

      <div>
        {theme ? <FaMoon  style={{fontSize:"16px", marginRight:"10px"}} onClick={changeTheme}/> : <CiLight className='icon' onClick={changeTheme}/>}
